...
var letterElemRef = false; // useful regular expressions
...
var ABRegExp = /[אבגדהוזחטיכךלמםנןסעפףצץקרשת]/g; // all dom elements
...
var DraftUpdateButton = undefined; //var SelectAll = undefined;
...
function show_to_user(text) {
  Status.val(text); //Status.text(text);
}
...
function naked(sentWords, successHandler, doneHandler, errorHandler) {
  // Update status
  show_to_user('מנקד...'); // Go to the server with words & their respective IDs

  $.post({
    url: 'app/naked',
    data: JSON.stringify(sentWords),
    success: function success(replyWords) {
      // reset status
      show_to_user('');
      ...
      for (var i = 0; i < replyWords.length; i++) {
        ...
        if (nikudim.length > 0) {
          ...
          show_to_user('השרת לא מצא ניקוד למילה [' + original + ']');
        } // Add naked word to the punctuations at the end


        nikudim.push(original); // Call supplied success handler to handle naked punctuation
...
function Suggest() {
  var inaked = Quicky.val(); // Do not suggest when the words are too short or havent changed
  ...
  show_to_user('אוטומט: ' + inaked); // Increment suggestions counter so we can connect
  // request to appropriate reply

  suggestID = suggestID + 1; // Go to the server with the word & their respective IDs
  ...
  $.post({
    ...
    success: function success(replyWord) {
      // reset status
      show_to_user(''); // Clear the old
      ...
      if (ID === suggestID && nakeds.length > 0) {
        ...
        SuggestionBox.fadeIn('slow'); // Add all suggestions to suggestion list
    ...
    error: function error(jqXHR, textStatus, errorThrown) {
      fake_use(textStatus);
      fake_use(jqXHR); // Update status as failure
...
function print_nikudim(nikudim) {
  ...
  var spElem = print_nikud(nikudim[0]); // Set title attribute to hold all possible punctuations

  spElem.attr('title', nikudim); // Set unique ID data of the span element

  runningID = runningID + 1;
  spElem.data('ID', runningID); // Return the span dom element
...
function cycle_nikud() {
  ...
  var nikudim = trim($(origElemRef).attr('title')).split(','); // Find the current punctuation in the punctuations list
...
function change_nikud(elem) {
  var newNikud = trim($(elem).text()); // If we have an orig element referece then update it
...
function nikudize(doneHandler) {
  ...
  SuggestionList.children('li').remove();
  Quicky.val('');
  Draft.text(''); // Hide all other punctuation ways punctuate automatic and focus on text
  ...
  Answer.text(''); // Build word list array (sent to server)
  ...
  while (true) {
    ...
    var word = do_enders(txt.substr(0, last)); // Add a span element containing the word to the answer
  ...
  naked(sentWords, function (nikudim, nikudID) {
...
function set_QuickyAns(nikudim, nikud, ID) {
  ...
  QuickyAns.text(''); // Add all possible punctuations to the QuickyAns div with the
  // origElemRef ID
  ...
  var word = trim($(draftElemRef).text()); // Update quicky with naked word

  SuggestionList.children('li').remove();
  Quicky.val(word.replace(nikudRegExp, '')); // Update draft with punc word
...
function set_Draft(nikud) {
  ...
  show_to_user(''); // Add each letter as a span element to the draft div
  // Remove current letters

  Draft.children('span').remove();
  Draft.text(''); // Traverse the word breaking it up into letters
  ...
  while (true) {
    ...
    Draft.append(print_nikud(brokenWord.substr(pos, posLast))); // Move to next letter

    brokenWord = brokenWord.substr(posLast);
  }

  letterElemRef = Draft.children(':first');
  $(letterElemRef).css('font-weight', 'bold'); // Iterate over the quicky ans words and bold just the selected word
...
function update_Draft() {
  ...
  current = do_enders(current); // Check if this punctuation exists
  ...
  if (exists) {
    ...
    $(exists).css('font-weight', 'bold'); // Change current nikud of orig element reference
  ...
  var currentSpan = print_nikud(current); // Append the new punctuation span to the quickyAns punctuation option word
  // span list and make it the chosen word

  currentSpan.css('font-weight', 'bold');
  QuickyAns.append(currentSpan);
  QuickyAns.append(' '); // If we are working on a word from the current text
...
function insert_Draft(value, letter) {
  ...
  var current = $(letter).text(); // The idea is that there could be a shin sin sound, a movement and a
...
function do_enders(word) {
  ...
  word = word.replace(/\u05e3/g, "\u05E4");
  word = word.replace(/\u05e5/g, "\u05E6");
  word = word.replace(/\u05DB[\u05B0\u05B1\u05B2\u05B3\u05B4\u05B5\u05B6\u05B7\u05C2\u05C1\u05B9\u05BC\u05BB]*$/, "\u05DA");
  word = word.replace(/\u05DB\u05B8$/, "\u05DA\u05B8");
  word = word.replace(/\u05DE[\u05B0\u05B1\u05B2\u05B3\u05B4\u05B5\u05B6\u05B7\u05B8\u05C2\u05C1\u05B9\u05BC\u05BB]*$/, "\u05DD");
  word = word.replace(/\u05E0[\u05B0\u05B1\u05B2\u05B3\u05B4\u05B5\u05B6\u05B7\u05B8\u05C2\u05C1\u05B9\u05BC\u05BB]*$/, "\u05DF");
  word = word.replace(/\u05E4[\u05B0\u05B1\u05B2\u05B3\u05B4\u05B5\u05B6\u05B7\u05B8\u05C2\u05C1\u05B9\u05BC\u05BB]*$/, "\u05E3");
  word = word.replace(/\u05E6[\u05B0\u05B1\u05B2\u05B3\u05B4\u05B5\u05B6\u05B7\u05B8\u05C2\u05C1\u05B9\u05BC\u05BB]*$/, "\u05E5");
  return word;
...
$(document).ready(function () {
  ...
  DraftUpdateButton = $('#DraftUpdateButton'); //SelectAll = $('#SelectAll');

  Status = $('#Status'); // Sometimes the browsers decide to save the latest state of the inputs
  // so override them

  MainText.removeAttr('disabled');
  NikudizeButton.removeAttr('disabled');
  Quicky.removeAttr('disabled');
  QuickyButton.removeAttr('disabled'); // Insert an introductry text to be punctuated

  MainText.val('ברוכים הבאים לנִקֻּדַהּ!\n' + 'לחצו על המלים כאן וראו כיצד הן משתנות.\n' + 'לעזרה נסו את הצרוף ctrl+shift+י\'.\nשמוש מצלח!'); // Bind letter and help hotkeys
  ...
  MainText.keypress(validate_Hebrew); // Setup the Answer click event to handle all text word clicks

  Answer.click(function (evt) {
    if ($(evt.target).is('span')) {
      HelpBox.hide('slow'); // Change the original element reference and bold it
  ...
  Quicky.keypress(validate_Hebrew); // Setup the QuickyAns click event to handle all punctuation option
  // word clicks

  QuickyAns.click(function (evt) {
    if ($(evt.target).is('span')) {
      HelpBox.hide('slow'); // Set the answer punctuation and set the draft area appropriately
  ...
  Quicky.focus(function (evt) {
    fake_use(evt); // Only fade in the suggestion box if there are items there
  ...
  SuggestionList.click(function (evt) {
    if ($(evt.target).is('li')) {
      ...
      SuggestionList.children('li').remove();
      SuggestionBox.hide('slow'); // Insert suggestions to quicky box and update
  ...
  nikudize(function () {
    // Select 12th word
    var mushtanot = Answer.children('span:nth-child(12)'); // Click it twice in order to change it to the correct punctuation
